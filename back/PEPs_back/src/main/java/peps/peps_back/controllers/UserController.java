...
public class UserController {
    ...
    public ResponseEntity<List<UserDTO>> getAllUsers() {
        List<UserDTO> users = userRepository.findAll().stream()
                .sorted((u1, u2) -> {
                    // Priority: Admin (3) > Editor (2) > Viewer (1)
                    int s1 = "admin".equalsIgnoreCase(u1.getRole()) ? 3
                            : "editor".equalsIgnoreCase(u1.getPermission()) ? 2 : 1;
                    int s2 = "admin".equalsIgnoreCase(u2.getRole()) ? 3
                            : "editor".equalsIgnoreCase(u2.getPermission()) ? 2 : 1;
    ...
    public ResponseEntity<UserDTO> updateUser(@PathVariable Integer id, @RequestBody UpdateUserRequest request,
        ...
        return userRepository.findById(id)
                .map(user -> {
                    ...
                    String newValue = String.format(
                            "{\"login\":\"%s\",\"role\":\"%s\",\"permission\":\"%s\"}",
                            updatedUser.getLogin(), updatedUser.getRole(), updatedUser.getPermission());
                    String userLogin = (login != null) ? login : "unknown";
                    auditService.log("UPDATE", "user", updatedUser.getIdUser(), updatedUser.getLogin(),
                            updatedUser.getRole(), userLogin, oldValue, newValue, "Modification de l'utilisateur");
        ...
        return userRepository.findById(id)
                .map(user -> {
                    ...
                    auditService.log("DELETE", "user", id, user.getLogin(),
                            user.getRole(), userLogin, oldValue, null, "Suppression de l'utilisateur");
        ...
        return userRepository.findById(id)
                .map(user -> {
                    ...
                    auditService.log("UPDATE", "user", user.getIdUser(), user.getLogin(),
                            user.getRole(), userLogin, oldValue, null, "Changement de mot de passe");
